...
function Helpers.renderDocumentHeader(state, emptyStateLabel, module)
	if imgui.Checkbox(u8"Включено", state.enabled) then
		Save.execute()
	end
	imgui.SameLine()
	if imgui.Checkbox(u8"Не менять чужие документы", state.onlyOwn) then
		Save.execute()
	end
	if emptyStateLabel and state.showEmptyState then
		imgui.SameLine()
		if imgui.Checkbox(emptyStateLabel, state.showEmptyState) then
			Save.execute()
		end
	end
...
function Helpers.renderFieldTable(labels, state)
	...
	if imgui.Button(u8"Включить все") then
		for _, item in ipairs(labels) do
			if state[item.key] then
				state[item.key].enabled[0] = true
			end
		end
		Save.execute()
	end
	...
	if imgui.Button(u8"Выключить все") then
		for _, item in ipairs(labels) do
			if state[item.key] then
				state[item.key].enabled[0] = false
			end
		end
		Save.execute()
	end

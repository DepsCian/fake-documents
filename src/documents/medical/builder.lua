...
local FIELD_MAP = { {
	...
	stateKey = "insurance",
	jsonKey = "health_insurance",
}, {
	stateKey = "validity",
	jsonKey = "med_card_time",
} }
...
function Builder.build()
	...
	local json =
		'{"type":4,"not":false' .. JsonBuilder.buildFields(s, FIELD_MAP)

	if s.psychiatric.enabled[0] then
		local value =
			require("core/encoding").escape(ffi.string(s.psychiatric.value))

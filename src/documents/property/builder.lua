...
function Builder.build()
    ...
    for i = 1, 5 do
        ...
        if slot.enabled[0] and slot.name.enabled[0] then
            table.insert(properties, '{"text":"' .. escape(ffi.string(slot.name.value)) .. '","column":"residential","property_type":"house"}')
        end
    end
    ...
    for i = 1, 5 do
        ...
        if slot.enabled[0] and slot.name.enabled[0] then
            table.insert(properties, '{"text":"' .. escape(ffi.string(slot.name.value)) .. '","column":"commerce","property_type":"house"}')
        end
    end

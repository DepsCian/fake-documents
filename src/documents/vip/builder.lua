...
function Builder.build()
    ...
    if s.statusName.enabled[0] then 
        local name = escape(ffi.string(s.statusName.value))
        local vip_type = vip_type_map[name] or "diamond"
        json = json .. '"type_vip":"' .. vip_type .. '"'
    end
    if s.statusTime.enabled[0] then json = json .. ',"vip_date":"' .. escape(ffi.string(s.statusTime.value)) .. '"' end
    if s.addVipDays.enabled[0] then json = json .. ',"add_vip_date":"' .. escape(ffi.string(s.addVipDays.value)) .. '"' end
    json = json .. '}'
    ...
    for i = 1, 10 do
        ...
        if lvl.enabled[0] then
            ...
            local text = escape(ffi.string(lvl.text.value))
            local desc = escape(ffi.string(lvl.desc.value))
            table.insert(levels, '{"mark":' .. mark .. ',"text":"' .. text .. '","desc":"' .. desc .. '"}')
        end
    end
    ...
    for i = 1, 4 do
        ...
        local type_name = escape(ffi.string(st.type.value))
        local type_vip = vip_type_map[type_name] or "diamond"
        local date_vip = escape(ffi.string(st.date.value))
        local available = st.enabled[0] and "true" or "false"
        table.insert(statuses, '{"type_vip":"' .. type_vip .. '","date_vip":"' .. date_vip .. '","available":' .. available .. '}')
    end

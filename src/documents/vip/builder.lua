...
function Builder.build()
    ...
    if s.statusTime.enabled[0] then json = json .. ',"vip_date":"' .. Encoding.escape(ffi.string(s.statusTime.value)) .. '"' end
    if s.addVipDays.enabled[0] then json = json .. ',"add_vip_date":"' .. Encoding.escape(ffi.string(s.addVipDays.value)) .. '"' end
    json = json .. '}'
